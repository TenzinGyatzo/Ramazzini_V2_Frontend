...
<template>
  <!-- Formulario Paso 1 -->
    <div v-if="registroExitoso == false"
      ...
      <FormKit type="form" :actions="false" incomplete-message="Por favor, valide que los datos sean correctos*"
        @submit="handleSubmit">
        <FormKit type="text" label="Nombre" name="username" placeholder="Ej. Jorge González" validation="required"
          :validation-messages="{
            required: 'Este campo es obligatorio'
          }" v-model="formDataUser.username" />

        <FormKit type="email" label="Correo de usuario" name="email" placeholder="usuario@empresa.com"
        ...
        <FormKit type="text" label="Teléfono de usuario" name="phone" placeholder="Teléfono"
        ...
        <FormKit type="password" label="Establece una contraseña" name="password" placeholder="Contraseña de usuario"
    ...
    <div v-else class="flex flex-col items-center justify-center p-6 w-full max-w-lg mx-auto">
      <!-- Mensaje de éxito -->
      <div class="text-center mb-2">
        <h2 class="text-2xl font-bold text-emerald-600 mb-4">Registro de usuario exitoso</h2>
        <p class="text-gray-600 mb-3">
          Has registrado el usuario correctamente. <strong>Es necesario activar la cuenta para poder iniciar sesión.</strong> Se ha enviado un correo de confirmación al usuario.
        </p>
      </div>

      <!-- Botón de redirección al inicio -->
      <div class="mt-2">
        <button @click="router.push({ name: 'inicio' })"
