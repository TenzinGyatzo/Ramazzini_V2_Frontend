  ...
  <div class="modal fixed top-0 left-0 z-10 p-8 h-screen w-full grid place-items-center">
    ...
    <Transition appear name="fade">
      ...
      <div class="modal-inner relative bg-white text-gray-900 w-full sm:w-4/5 md:w-3/5 xl:w-2/5 2xl:w-1/3 p-10 rounded-lg shadow-md shadow-slate-900 max-h-[90vh] overflow-y-auto">
        ...
        <!-- Contenido del modal -->
        <div v-else>
          <h1 class="text-3xl">{{ trabajadores.currentTrabajador._id ? 'Editar Trabajador' : 'Registrar Trabajador' }}</h1>
          <hr class="mt-2 mb-3">
          ...
          >
            <div class="lg:grid gap-4 lg:grid-cols-2">
              ...
              <FormKit 
                ...
                label="Escolaridad*"
                name="escolaridad"
                placeholder="Último nivel de escolaridad concluida"
                :options="nivelesEscolaridad"
                validation="required"
                :validation-messages="{ required: 'Este campo es obligatorio'}"
              ...
              <FormKit 
                ...
                validation="optional|length:10"
                :validation-messages="{ length: 'El teléfono debe tener 10 dígitos' }"
              ...
              <FormKit
                ...
                max="10"
                step="1"
                :value="trabajadores.currentTrabajador?.hijos || 0"
              />
            </div>
            ...
            <FormKit 
              ...
              :value="centrosTrabajo.currentCentroTrabajoId"
            />
            <FormKit 
              type="hidden"
              name="createdBy"
            ...
            <FormKit 
              type="submit"
              :disabled="trabajadores.loadingModal" 
            >
              <span v-if="trabajadores.loadingModal">Guardando...</span>
              <span v-else>{{ trabajadores.currentTrabajador._id ? 'Actualizar Trabajador' : 'Guardar Trabajador' }}</span>
            </FormKit>
          </FormKit>
        </div>
