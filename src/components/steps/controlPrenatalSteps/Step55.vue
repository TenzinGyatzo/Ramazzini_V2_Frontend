...
onMounted(() => {
  ...
  if (formDataControlPrenatal.julioPeso) {
    julioPeso.value = formDataControlPrenatal.julioPeso;
  }
  if (formDataControlPrenatal.julioImc) {
    julioImc.value = formDataControlPrenatal.julioImc;
  }
...
const puedeCalcularIMC = computed(() => {
  return julioPeso.value && alturaDisponible.value;
});

// Validaciones reactivas similares a Step2.vue
const mensajeErrorPeso = computed(() => {
  if (!julioPeso.value || julioPeso.value === '') return '';
  
  const peso = parseFloat(julioPeso.value);
...
<template>
  <div>
    ...
    <div class="mb-6">
      ...
      <p class="font-medium mb-2 text-gray-800 leading-5">¿Cuál fue el peso registrado durante el control prenatal del mes de julio?</p>
    ...
    <div class="mb-6">
      ...
      <div v-else-if="julioImc" class="mt-1 p-3 bg-emerald-50 border border-emerald-200 rounded-lg">
          <p class="text-sm text-emerald-700">
              ✅ IMC calculado: <span class="font-semibold">{{ julioImc }}</span>
          </p>
      </div>
      ...
      <div class="font-light mt-2">
        <input type="number"
            ...
            :disabled="!julioPeso || !alturaDisponible"
            readonly>
      </div>
