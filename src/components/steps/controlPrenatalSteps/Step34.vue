...
onMounted(() => {
  ...
  if (formDataControlPrenatal.abrilPeso) {
    abrilPeso.value = formDataControlPrenatal.abrilPeso;
  }
  if (formDataControlPrenatal.abrilImc) {
    abrilImc.value = formDataControlPrenatal.abrilImc;
  }
...
onUnmounted(() => {
  ...
  formDataControlPrenatal.abrilPeso = abrilPeso.value || '';
  formDataControlPrenatal.abrilImc = abrilImc.value || '';
  if (alturaLocal.value) {
    formDataControlPrenatal.altura = alturaLocal.value;
  }
});

// Sincronizar abrilPeso con formData
watch(abrilPeso, (newValue) => {
  formDataControlPrenatal.abrilPeso = newValue;
  ...
  if (newValue && alturaDisponible.value) {
    ...
    abrilImc.value = imcCalculado;
    formDataControlPrenatal.abrilImc = imcCalculado;
  } else {
    abrilImc.value = '';
    formDataControlPrenatal.abrilImc = '';
  }
});
...
watch(alturaLocal, (newAltura) => {
  if (newAltura) {
    ...
    if (abrilPeso.value) {
      const imcCalculado = calcularIMC(abrilPeso.value, newAltura);
      abrilImc.value = imcCalculado;
      formDataControlPrenatal.abrilImc = imcCalculado;
    }
  }
...
const puedeCalcularIMC = computed(() => {
  return abrilPeso.value && alturaDisponible.value;
});

// Validaciones reactivas similares a Step2.vue
const mensajeErrorPeso = computed(() => {
  if (!abrilPeso.value || abrilPeso.value === '') return '';
  
  const peso = parseFloat(abrilPeso.value);
...
<template>
  <div>
    ...
    <div class="mb-6">
      ...
      <p class="font-medium mb-2 text-gray-800 leading-5">¿Cuál fue el peso registrado durante el control prenatal del mes de abril?</p>
    ...
    <div class="mb-6">
      ...
      <div v-else-if="abrilImc" class="mt-1 p-3 bg-emerald-50 border border-emerald-200 rounded-lg">
          <p class="text-sm text-emerald-700">
              ✅ IMC calculado: <span class="font-semibold">{{ abrilImc }}</span>
          </p>
      </div>
      ...
      <div class="font-light mt-2">
        <input type="number"
            ...
            :disabled="!abrilPeso || !alturaDisponible"
            readonly>
      </div>
