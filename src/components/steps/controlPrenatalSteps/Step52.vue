...
onMounted(() => {
  // Verificar si formDataControlPrenatal.julioSdg tiene un valor y establecerlo
  if (formDataControlPrenatal.julioSdg) {
    julioSdg.value = formDataControlPrenatal.julioSdg.toString();
  }
});

onUnmounted(() => {
  // Asegurar que formData tenga un valor inicial para julioSdg
  if (!formDataControlPrenatal.julioSdg) {
    formDataControlPrenatal.julioSdg = julioSdg.value ? parseInt(julioSdg.value) : undefined;
  }
});

// Sincronizar julioSdg con formData
watch(julioSdg, (newValue) => {
  if (newValue && newValue !== '') {
    formDataControlPrenatal.julioSdg = parseInt(newValue);
  } else {
    formDataControlPrenatal.julioSdg = undefined;
  }
});

// Computed para la categoría de gestación
const categoriaGestacionComputed = computed(() => {
  return determinarCategoriaGestacion(julioSdg.value);
});
...
const mensajeErrorSemana = computed(() => {
  if (!julioSdg.value || julioSdg.value === '') return '';
  
  const semana = parseInt(julioSdg.value);
...
<template>
  <div>
      ...
      <div class="mb-4">
        ...
        <div class="mb-4 mt-4">
          <div class="grid grid-cols-4 gap-3">
            <button
              ...
              :class="julioSdg === preset.valor.toString() 
        ...
        <div class="mb-4">
          <div class="flex items-center justify-center gap-3">
            <label for="julioSdg" class="font-medium text-gray-700">Personalizar:</label>
            <input
              id="julioSdg"
              v-model="julioSdg"
              type="number"
    ...
    <div class="mb-6">
      <div v-if="julioSdg && !mensajeErrorSemana" class="mb-4">
      ...
      <div v-else-if="julioSdg && mensajeErrorSemana" class="p-3 bg-red-50 border border-red-200 rounded-lg">
