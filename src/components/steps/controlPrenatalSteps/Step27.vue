...
onMounted(() => {
  // Verificar si formDataControlPrenatal.marzoFcf tiene un valor y establecerlo
  if (formDataControlPrenatal.marzoFcf) {
    marzoFcf.value = formDataControlPrenatal.marzoFcf.toString();
  }
});

onUnmounted(() => {
  // Asegurar que formData tenga un valor inicial para marzoFcf
  if (!formDataControlPrenatal.marzoFcf) {
    formDataControlPrenatal.marzoFcf = marzoFcf.value ? parseInt(marzoFcf.value) : undefined;
  }
});

// Sincronizar marzoFcf con formData
watch(marzoFcf, (newValue) => {
  if (newValue && newValue !== '') {
    formDataControlPrenatal.marzoFcf = parseInt(newValue);
  } else {
    formDataControlPrenatal.marzoFcf = undefined;
  }
});

// Computed para la categoría de FCF
const categoriaFcfComputed = computed(() => {
  return determinarCategoriaFcf(marzoFcf.value);
});

// Validaciones reactivas
const mensajeErrorFcf = computed(() => {
  if (!marzoFcf.value || marzoFcf.value === '') return '';
  
  const fcf = parseInt(marzoFcf.value);
...
<template>
  <div>
      ...
      <div class="mb-4">
        ...
        <div class="mb-4 mt-4">
          <div class="grid grid-cols-5 gap-3">
            <button
              ...
              :class="marzoFcf === preset.valor.toString() 
        ...
        <div class="mb-4">
          <div class="flex items-center justify-center gap-3">
            <label for="marzoFcf" class="font-medium text-gray-700">Personalizar:</label>
            <input
              id="marzoFcf"
              v-model="marzoFcf"
              type="number"
    ...
    <div class="mb-6">
      ...
      <div v-else-if="marzoFcf && mensajeErrorFcf" class="p-3 bg-red-50 border border-red-200 rounded-lg">
