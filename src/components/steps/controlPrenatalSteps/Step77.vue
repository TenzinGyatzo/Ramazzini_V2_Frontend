...
// Valores locales
const noviembreFondoUterino = ref('');
...
onMounted(() => {
  // Verificar si formDataControlPrenatal.noviembreFondoUterino tiene un valor y establecerlo
  if (formDataControlPrenatal.noviembreFondoUterino) {
    noviembreFondoUterino.value = formDataControlPrenatal.noviembreFondoUterino.toString();
  }
});

onUnmounted(() => {
  // Asegurar que formData tenga un valor inicial para noviembreFondoUterino
  if (!formDataControlPrenatal.noviembreFondoUterino) {
    formDataControlPrenatal.noviembreFondoUterino = noviembreFondoUterino.value ? parseFloat(noviembreFondoUterino.value) : undefined;
  }
});

// Sincronizar noviembreFondoUterino con formData
watch(noviembreFondoUterino, (newValue) => {
  if (newValue && newValue !== '') {
    formDataControlPrenatal.noviembreFondoUterino = parseFloat(newValue);
  } else {
    formDataControlPrenatal.noviembreFondoUterino = undefined;
  }
});

// Computed para la categoría del fondo uterino
const categoriaFondoUterinoComputed = computed(() => {
  return determinarCategoriaFondoUterino(noviembreFondoUterino.value, formDataControlPrenatal.noviembreSdg);
});

// Computed para el fondo uterino esperado
const fondoUterinoEsperadoComputed = computed(() => {
  return calcularFondoUterinoEsperado(formDataControlPrenatal.noviembreSdg);
});

// Validaciones reactivas
const mensajeErrorFondoUterino = computed(() => {
  if (!noviembreFondoUterino.value || noviembreFondoUterino.value === '') return '';
  
  const fondoUterino = parseFloat(noviembreFondoUterino.value);
...
const seleccionarPreset = (valor) => {
  noviembreFondoUterino.value = valor.toString();
};
...
<template>
  <div>
      ...
      <div class="mb-4">
        ...
        <div class="mb-4 mt-4">
          <div class="grid grid-cols-3 gap-3">
            <button
              ...
              :class="noviembreFondoUterino === preset.valor.toString() 
        ...
        <div class="mb-4">
          <div class="flex items-center justify-center gap-3">
            <label for="noviembreFondoUterino" class="font-medium text-gray-700">Personalizar:</label>
            <input
              id="noviembreFondoUterino"
              v-model="noviembreFondoUterino"
              type="number"
              min="0"
              max="50"
              step="0.5"
    ...
    <div class="mb-4">
      ...
      <div v-else-if="noviembreFondoUterino && mensajeErrorFondoUterino" class="p-3 bg-red-50 border border-red-200 rounded-lg">
    ...
    <div v-if="fondoUterinoEsperadoComputed && formDataControlPrenatal.noviembreSdg" class="mb-2">
