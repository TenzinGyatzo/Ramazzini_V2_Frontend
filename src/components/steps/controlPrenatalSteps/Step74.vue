<template>
  <div class="step-container">
    <h2 class="step-title">Control Prenatal - Noviembre - Tensión Arterial</h2>
...
onMounted(() => {
  if ((formDataStore.formDataControlPrenatal as any).noviembreTia) {
    tensionArterialCompleta.value = (formDataStore.formDataControlPrenatal as any).noviembreTia
    // Extraer valores individuales si es necesario
    const partes = (formDataStore.formDataControlPrenatal as any).noviembreTia.split('/')
    if (partes.length === 2) {
...
watch([tensionArterialCompleta], (newValue) => {
  if (newValue && !mensajeErrorRelacion.value) {
    (formDataStore.formDataControlPrenatal as any).noviembreTia = newValue
  }
}, { deep: true })
