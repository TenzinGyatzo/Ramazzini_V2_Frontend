...
onMounted(() => {
  // Verificar si formDataControlPrenatal tiene valores para noviembre y establecerlos
  if (formDataControlPrenatal.noviembreTia) {
    // Extraer sistólica y diastólica del formato "120/80"
    const tensionCompleta = formDataControlPrenatal.noviembreTia;
...
onUnmounted(() => {
  // Guardar la tensión arterial completa en el campo de noviembre
  if (tensionArterialCompleta.value) {
    formDataControlPrenatal.noviembreTia = tensionArterialCompleta.value;
  } else {
    formDataControlPrenatal.noviembreTia = null;
  }
});
...
watch([tensionArterialSistolica, tensionArterialDiastolica], ([sistolica, diastolica]) => {
  ...
  // Guardar en el campo de noviembre cuando se complete
  if (tensionCompleta) {
    formDataControlPrenatal.noviembreTia = tensionCompleta;
  } else {
    formDataControlPrenatal.noviembreTia = null;
  }
});
