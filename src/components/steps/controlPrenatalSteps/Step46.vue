...
onMounted(() => {
  // Verificar si formDataControlPrenatal.junioSdg tiene un valor y establecerlo
  if (formDataControlPrenatal.junioSdg) {
    junioSdg.value = formDataControlPrenatal.junioSdg.toString();
  }
});

onUnmounted(() => {
  // Asegurar que formData tenga un valor inicial para junioSdg
  if (!formDataControlPrenatal.junioSdg) {
    formDataControlPrenatal.junioSdg = junioSdg.value ? parseInt(junioSdg.value) : undefined;
  }
});

// Sincronizar junioSdg con formData
watch(junioSdg, (newValue) => {
  if (newValue && newValue !== '') {
    formDataControlPrenatal.junioSdg = parseInt(newValue);
  } else {
    formDataControlPrenatal.junioSdg = undefined;
  }
});

// Computed para la categoría de gestación
const categoriaGestacionComputed = computed(() => {
  return determinarCategoriaGestacion(junioSdg.value);
});
...
const mensajeErrorSemana = computed(() => {
  if (!junioSdg.value || junioSdg.value === '') return '';
  
  const semana = parseInt(junioSdg.value);
...
<template>
  <div>
      ...
      <div class="mb-4">
        ...
        <div class="mb-4 mt-4">
          <div class="grid grid-cols-4 gap-3">
            <button
              ...
              :class="junioSdg === preset.valor.toString() 
        ...
        <div class="mb-4">
          <div class="flex items-center justify-center gap-3">
            <label for="junioSdg" class="font-medium text-gray-700">Personalizar:</label>
            <input
              id="junioSdg"
              v-model="junioSdg"
              type="number"
    ...
    <div class="mb-6">
      <div v-if="junioSdg && !mensajeErrorSemana" class="mb-4">
      ...
      <div v-else-if="junioSdg && mensajeErrorSemana" class="p-3 bg-red-50 border border-red-200 rounded-lg">
