...
onMounted(() => {
  // Verificar si formDataControlPrenatal.abrilFcf tiene un valor y establecerlo
  if (formDataControlPrenatal.abrilFcf) {
    abrilFcf.value = formDataControlPrenatal.abrilFcf.toString();
  }
});
...
const categoriaFcfComputed = computed(() => {
  return determinarCategoriaFcf(abrilFcf.value);
});

// Validaciones reactivas
const mensajeErrorFcf = computed(() => {
  if (!abrilFcf.value || abrilFcf.value === '') return '';
  
  const fcf = parseInt(abrilFcf.value);
...
<template>
  <div>
    ...
    <div class="mb-6">
      ...
      <div v-else-if="abrilFcf && mensajeErrorFcf" class="p-3 bg-red-50 border border-red-200 rounded-lg">
