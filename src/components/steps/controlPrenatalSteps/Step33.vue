<template>
  <div class="step-container">
    <h2 class="step-title">Control Prenatal - Abril - Frecuencia Cardíaca Fetal</h2>
    
    <div class="form-section">
      <div class="input-group">
        <label for="abrilFcf" class="form-label">Frecuencia Cardíaca Fetal (FCF) - Abril</label>
...
onMounted(() => {
  if ((formDataStore.formDataControlPrenatal as any).abrilFcf) {
    abrilFcf.value = (formDataStore.formDataControlPrenatal as any).abrilFcf
    categoriaFcf.value = determinarCategoriaFcf(Number((formDataStore.formDataControlPrenatal as any).abrilFcf))
  }
})
...
watch([abrilFcf], (newValue) => {
  if (newValue && !mensajeErrorFcf.value) {
    (formDataStore.formDataControlPrenatal as any).abrilFcf = newValue
...
const categoriaFcfComputed = computed(() => {
  const fcf = Number(abrilFcf.value)
  if (!abrilFcf.value) return { text: 'No especificado', class: 'text-gray-500' }
...
const mensajeErrorFcf = computed(() => {
  if (!abrilFcf.value) return 'La FCF es requerida'
  const fcf = Number(abrilFcf.value)
