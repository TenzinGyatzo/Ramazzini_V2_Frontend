...
onMounted(() => {
  // Verificar si formDataControlPrenatal.marzoSdg tiene un valor y establecerlo
  if (formDataControlPrenatal.marzoSdg) {
    marzoSdg.value = formDataControlPrenatal.marzoSdg.toString();
  }
});

onUnmounted(() => {
  // Asegurar que formData tenga un valor inicial para marzoSdg
  if (!formDataControlPrenatal.marzoSdg) {
    formDataControlPrenatal.marzoSdg = marzoSdg.value ? parseInt(marzoSdg.value) : undefined;
  }
});

// Sincronizar marzoSdg con formData
watch(marzoSdg, (newValue) => {
  if (newValue && newValue !== '') {
    formDataControlPrenatal.marzoSdg = parseInt(newValue);
  } else {
    formDataControlPrenatal.marzoSdg = undefined;
  }
});

// Computed para la categoría de gestación
const categoriaGestacionComputed = computed(() => {
  return determinarCategoriaGestacion(marzoSdg.value);
});
...
const mensajeErrorSemana = computed(() => {
  if (!marzoSdg.value || marzoSdg.value === '') return '';
  
  const semana = parseInt(marzoSdg.value);
...
<template>
  <div>
      ...
      <div class="mb-4">
        ...
        <div class="mb-4 mt-4">
          <div class="grid grid-cols-4 gap-3">
            <button
              ...
              :class="marzoSdg === preset.valor.toString() 
        ...
        <div class="mb-4">
          <div class="flex items-center justify-center gap-3">
            <label for="marzoSdg" class="font-medium text-gray-700">Personalizar:</label>
            <input
              id="marzoSdg"
              v-model="marzoSdg"
              type="number"
    ...
    <div class="mb-6">
      <div v-if="marzoSdg && !mensajeErrorSemana" class="mb-4">
      ...
      <div v-else-if="marzoSdg && mensajeErrorSemana" class="p-3 bg-red-50 border border-red-200 rounded-lg">
