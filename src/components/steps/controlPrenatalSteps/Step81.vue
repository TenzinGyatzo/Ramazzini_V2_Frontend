...
onMounted(() => {
  // Verificar si formDataControlPrenatal.diciembreFcf tiene un valor y establecerlo
  if (formDataControlPrenatal.diciembreFcf) {
    diciembreFcf.value = formDataControlPrenatal.diciembreFcf.toString();
  }
});

onUnmounted(() => {
  // Asegurar que formData tenga un valor inicial para diciembreFcf
  if (!formDataControlPrenatal.diciembreFcf) {
    formDataControlPrenatal.diciembreFcf = diciembreFcf.value ? parseInt(diciembreFcf.value) : undefined;
  }
});

// Sincronizar diciembreFcf con formData
watch(diciembreFcf, (newValue) => {
  if (newValue && newValue !== '') {
    formDataControlPrenatal.diciembreFcf = parseInt(newValue);
  } else {
    formDataControlPrenatal.diciembreFcf = undefined;
  }
});

// Computed para la categoría de FCF
const categoriaFcfComputed = computed(() => {
  return determinarCategoriaFcf(diciembreFcf.value);
});

// Validaciones reactivas
const mensajeErrorFcf = computed(() => {
  if (!diciembreFcf.value || diciembreFcf.value === '') return '';
  
  const fcf = parseInt(diciembreFcf.value);
...
const seleccionarPreset = (valor) => {
  diciembreFcf.value = valor.toString();
};
...
<template>
  <div>
      ...
      <div class="mb-4">
        ...
        <div class="mb-4 mt-4">
          <div class="grid grid-cols-5 gap-3">
            <button
              ...
              :class="diciembreFcf === preset.valor.toString() 
        ...
        <div class="mb-4">
          <div class="flex items-center justify-center gap-3">
            <label for="diciembreFcf" class="font-medium text-gray-700">Personalizar:</label>
            <input
              id="diciembreFcf"
              v-model="diciembreFcf"
              type="number"
              min="60"
              max="250"
              placeholder="Ej: 135"
    ...
    <div class="mb-6">
      ...
      <div v-else-if="diciembreFcf && mensajeErrorFcf" class="p-3 bg-red-50 border border-red-200 rounded-lg">
