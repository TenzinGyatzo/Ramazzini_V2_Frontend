<template>
  <div class="step-container">
    <h2 class="step-title">Control Prenatal - Diciembre - Frecuencia Cardíaca Fetal</h2>
    
    <div class="form-section">
      <div class="input-group">
        <label for="diciembreFcf" class="form-label">Frecuencia Cardíaca Fetal (FCF) - Diciembre</label>
...
onMounted(() => {
  ...
  if (savedData.diciembreFcf) {
    diciembreFcf.value = savedData.diciembreFcf
    categoriaFcf.value = determinarCategoriaFcf(Number(savedData.diciembreFcf))
  }
})
...
const categoriaFcfComputed = computed(() => {
  const fcf = Number(diciembreFcf.value)
  if (!diciembreFcf.value) return { text: 'No especificado', class: 'text-gray-500' }
...
const mensajeErrorFcf = computed(() => {
  if (!diciembreFcf.value) return 'La FCF es requerida'
  const fcf = Number(diciembreFcf.value)
