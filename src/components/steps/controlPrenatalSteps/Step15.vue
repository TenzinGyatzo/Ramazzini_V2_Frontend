...
onMounted(() => {
  // Verificar si formDataControlPrenatal.eneroFcf tiene un valor y establecerlo
  if (formDataControlPrenatal.eneroFcf) {
    eneroFcf.value = formDataControlPrenatal.eneroFcf.toString();
  }
});

onUnmounted(() => {
  // Asegurar que formData tenga un valor inicial para eneroFcf
  if (!formDataControlPrenatal.eneroFcf) {
    formDataControlPrenatal.eneroFcf = eneroFcf.value ? parseInt(eneroFcf.value) : undefined;
  }
});

// Sincronizar eneroFcf con formData
watch(eneroFcf, (newValue) => {
  if (newValue && newValue !== '') {
    formDataControlPrenatal.eneroFcf = parseInt(newValue);
  } else {
    formDataControlPrenatal.eneroFcf = undefined;
  }
});

// Computed para la categoría de FCF
const categoriaFcfComputed = computed(() => {
  return determinarCategoriaFcf(eneroFcf.value);
});

// Validaciones reactivas
const mensajeErrorFcf = computed(() => {
  if (!eneroFcf.value || eneroFcf.value === '') return '';
  
  const fcf = parseInt(eneroFcf.value);
...
<template>
  <div>
      ...
      <div class="mb-4">
        ...
        <div class="mb-4 mt-4">
          <div class="grid grid-cols-5 gap-3">
            <button
              ...
              :class="eneroFcf === preset.valor.toString() 
        ...
        <div class="mb-4">
          <div class="flex items-center justify-center gap-3">
            <label for="eneroFcf" class="font-medium text-gray-700">Personalizar:</label>
            <input
              id="eneroFcf"
              v-model="eneroFcf"
              type="number"
    ...
    <div class="mb-6">
      ...
      <div v-else-if="eneroFcf && mensajeErrorFcf" class="p-3 bg-red-50 border border-red-200 rounded-lg">
