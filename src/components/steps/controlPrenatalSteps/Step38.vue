...
onMounted(() => {
  // Verificar si formDataControlPrenatal tiene valores para mayo y establecerlos
  if (formDataControlPrenatal.mayoTia) {
    // Extraer sistólica y diastólica del formato "120/80"
    const tensionCompleta = formDataControlPrenatal.mayoTia;
...
onUnmounted(() => {
  // Guardar la tensión arterial completa en el campo de mayo
  if (tensionArterialCompleta.value) {
    formDataControlPrenatal.mayoTia = tensionArterialCompleta.value;
  } else {
    formDataControlPrenatal.mayoTia = null;
  }
});
...
watch([tensionArterialSistolica, tensionArterialDiastolica], ([sistolica, diastolica]) => {
  ...
  // Guardar en el campo de mayo cuando se complete
  if (tensionCompleta) {
    formDataControlPrenatal.mayoTia = tensionCompleta;
  } else {
    formDataControlPrenatal.mayoTia = null;
  }
});
