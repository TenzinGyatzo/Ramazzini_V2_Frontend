<template>
  <div class="step-container">
    <h2 class="step-title">Control Prenatal - Julio - Tensión Arterial</h2>
...
onMounted(() => {
  if ((formDataStore.formDataControlPrenatal as any).julioTia) {
    tensionArterialCompleta.value = (formDataStore.formDataControlPrenatal as any).julioTia
    // Extraer valores individuales si es necesario
    const partes = (formDataStore.formDataControlPrenatal as any).julioTia.split('/')
    if (partes.length === 2) {
...
watch([tensionArterialCompleta], (newValue) => {
  if (newValue && !mensajeErrorRelacion.value) {
    (formDataStore.formDataControlPrenatal as any).julioTia = newValue
  }
}, { deep: true })
