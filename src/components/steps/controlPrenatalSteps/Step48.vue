...
onMounted(() => {
  ...
  if (formDataControlPrenatal.junioPeso) {
    junioPeso.value = formDataControlPrenatal.junioPeso;
  }
  if (formDataControlPrenatal.junioImc) {
    junioImc.value = formDataControlPrenatal.junioImc;
  }
...
onUnmounted(() => {
  ...
  formDataControlPrenatal.junioPeso = junioPeso.value || '';
  formDataControlPrenatal.junioImc = junioImc.value || '';
  if (alturaLocal.value) {
    formDataControlPrenatal.altura = alturaLocal.value;
  }
});

// Sincronizar junioPeso con formData
watch(junioPeso, (newValue) => {
  formDataControlPrenatal.junioPeso = newValue;
  ...
  if (newValue && alturaDisponible.value) {
    ...
    junioImc.value = imcCalculado;
    formDataControlPrenatal.junioImc = imcCalculado;
  } else {
    junioImc.value = '';
    formDataControlPrenatal.junioImc = '';
  }
});
...
watch(alturaLocal, (newAltura) => {
  if (newAltura) {
    ...
    if (junioPeso.value) {
      const imcCalculado = calcularIMC(junioPeso.value, newAltura);
      junioImc.value = imcCalculado;
      formDataControlPrenatal.junioImc = imcCalculado;
    }
  }
...
const puedeCalcularIMC = computed(() => {
  return junioPeso.value && alturaDisponible.value;
});

// Validaciones reactivas similares a Step2.vue
const mensajeErrorPeso = computed(() => {
  if (!junioPeso.value || junioPeso.value === '') return '';
  
  const peso = parseFloat(junioPeso.value);
...
<template>
  <div>
    ...
    <div class="mb-6">
      ...
      <p class="font-medium mb-2 text-gray-800 leading-5">¿Cuál fue el peso registrado durante el control prenatal del mes de junio?</p>
    ...
    <div class="mb-6">
      ...
      <div v-else-if="junioImc" class="mt-1 p-3 bg-emerald-50 border border-emerald-200 rounded-lg">
          <p class="text-sm text-emerald-700">
              ✅ IMC calculado: <span class="font-semibold">{{ junioImc }}</span>
          </p>
      </div>
      ...
      <div class="font-light mt-2">
        <input type="number"
            ...
            :disabled="!junioPeso || !alturaDisponible"
            readonly>
      </div>
