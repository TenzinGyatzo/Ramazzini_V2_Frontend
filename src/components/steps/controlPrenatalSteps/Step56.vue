...
onMounted(() => {
  // Verificar si formDataControlPrenatal tiene valores para agosto y establecerlos
  if (formDataControlPrenatal.agostoTia) {
    // Extraer sistólica y diastólica del formato "120/80"
    const tensionCompleta = formDataControlPrenatal.agostoTia;
...
onUnmounted(() => {
  // Guardar la tensión arterial completa en el campo de agosto
  if (tensionArterialCompleta.value) {
    formDataControlPrenatal.agostoTia = tensionArterialCompleta.value;
  } else {
    formDataControlPrenatal.agostoTia = null;
  }
});
...
watch([tensionArterialSistolica, tensionArterialDiastolica], ([sistolica, diastolica]) => {
  ...
  // Guardar en el campo de agosto cuando se complete
  if (tensionCompleta) {
    formDataControlPrenatal.agostoTia = tensionCompleta;
  } else {
    formDataControlPrenatal.agostoTia = null;
  }
});
