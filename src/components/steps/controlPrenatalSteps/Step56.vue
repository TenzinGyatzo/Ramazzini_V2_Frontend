<template>
  <div class="step-container">
    <h2 class="step-title">Control Prenatal - Agosto - Tensión Arterial</h2>
...
onMounted(() => {
  if ((formDataStore.formDataControlPrenatal as any).agostoTia) {
    tensionArterialCompleta.value = (formDataStore.formDataControlPrenatal as any).agostoTia
    // Extraer valores individuales si es necesario
    const partes = (formDataStore.formDataControlPrenatal as any).agostoTia.split('/')
    if (partes.length === 2) {
...
watch([tensionArterialCompleta], (newValue) => {
  if (newValue && !mensajeErrorRelacion.value) {
    (formDataStore.formDataControlPrenatal as any).agostoTia = newValue
  }
}, { deep: true })
