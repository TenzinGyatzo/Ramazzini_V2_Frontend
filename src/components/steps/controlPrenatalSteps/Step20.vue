...
onMounted(() => {
  // Verificar si formDataControlPrenatal tiene valores para febrero y establecerlos
  if (formDataControlPrenatal.febreroTia) {
    // Extraer sistólica y diastólica del formato "120/80"
    const tensionCompleta = formDataControlPrenatal.febreroTia;
...
onUnmounted(() => {
  // Guardar la tensión arterial completa en el campo de febrero
  if (tensionArterialCompleta.value) {
    formDataControlPrenatal.febreroTia = tensionArterialCompleta.value;
  } else {
    formDataControlPrenatal.febreroTia = null;
  }
});
...
watch([tensionArterialSistolica, tensionArterialDiastolica], ([sistolica, diastolica]) => {
  ...
  // Guardar en el campo de febrero cuando se complete
  if (tensionCompleta) {
    formDataControlPrenatal.febreroTia = tensionCompleta;
  } else {
    formDataControlPrenatal.febreroTia = null;
  }
});
