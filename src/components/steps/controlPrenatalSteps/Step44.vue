<template>
  <div class="step-container">
    <h2 class="step-title">Control Prenatal - Junio - Tensión Arterial</h2>
...
onMounted(() => {
  if ((formDataStore.formDataControlPrenatal as any).junioTia) {
    tensionArterialCompleta.value = (formDataStore.formDataControlPrenatal as any).junioTia
    // Extraer valores individuales si es necesario
    const partes = (formDataStore.formDataControlPrenatal as any).junioTia.split('/')
    if (partes.length === 2) {
...
watch([tensionArterialCompleta], (newValue) => {
  if (newValue && !mensajeErrorRelacion.value) {
    (formDataStore.formDataControlPrenatal as any).junioTia = newValue
  }
}, { deep: true })
