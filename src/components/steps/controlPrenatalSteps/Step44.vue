...
onMounted(() => {
  // Verificar si formDataControlPrenatal tiene valores para junio y establecerlos
  if (formDataControlPrenatal.junioTia) {
    // Extraer sistólica y diastólica del formato "120/80"
    const tensionCompleta = formDataControlPrenatal.junioTia;
...
onUnmounted(() => {
  // Guardar la tensión arterial completa en el campo de junio
  if (tensionArterialCompleta.value) {
    formDataControlPrenatal.junioTia = tensionArterialCompleta.value;
  } else {
    formDataControlPrenatal.junioTia = null;
  }
});
...
watch([tensionArterialSistolica, tensionArterialDiastolica], ([sistolica, diastolica]) => {
  ...
  // Guardar en el campo de junio cuando se complete
  if (tensionCompleta) {
    formDataControlPrenatal.junioTia = tensionCompleta;
  } else {
    formDataControlPrenatal.junioTia = null;
  }
});
