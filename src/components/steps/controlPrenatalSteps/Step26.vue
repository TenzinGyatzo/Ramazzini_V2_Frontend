...
onMounted(() => {
  // Verificar si formDataControlPrenatal tiene valores para marzo y establecerlos
  if (formDataControlPrenatal.marzoTia) {
    // Extraer sistólica y diastólica del formato "120/80"
    const tensionCompleta = formDataControlPrenatal.marzoTia;
...
onUnmounted(() => {
  // Guardar la tensión arterial completa en el campo de marzo
  if (tensionArterialCompleta.value) {
    formDataControlPrenatal.marzoTia = tensionArterialCompleta.value;
  } else {
    formDataControlPrenatal.marzoTia = null;
  }
});
...
watch([tensionArterialSistolica, tensionArterialDiastolica], ([sistolica, diastolica]) => {
  ...
  // Guardar en el campo de marzo cuando se complete
  if (tensionCompleta) {
    formDataControlPrenatal.marzoTia = tensionCompleta;
  } else {
    formDataControlPrenatal.marzoTia = null;
  }
});
