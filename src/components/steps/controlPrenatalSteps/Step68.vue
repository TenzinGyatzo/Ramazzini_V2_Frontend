...
onMounted(() => {
  // Verificar si formDataControlPrenatal tiene valores para octubre y establecerlos
  if (formDataControlPrenatal.octubreTia) {
    // Extraer sistólica y diastólica del formato "120/80"
    const tensionCompleta = formDataControlPrenatal.octubreTia;
...
onUnmounted(() => {
  // Guardar la tensión arterial completa en el campo de octubre
  if (tensionArterialCompleta.value) {
    formDataControlPrenatal.octubreTia = tensionArterialCompleta.value;
  } else {
    formDataControlPrenatal.octubreTia = null;
  }
});
...
watch([tensionArterialSistolica, tensionArterialDiastolica], ([sistolica, diastolica]) => {
  ...
  // Guardar en el campo de octubre cuando se complete
  if (tensionCompleta) {
    formDataControlPrenatal.octubreTia = tensionCompleta;
  } else {
    formDataControlPrenatal.octubreTia = null;
  }
});
