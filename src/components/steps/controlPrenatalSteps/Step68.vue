<template>
  <div class="step-container">
    <h2 class="step-title">Control Prenatal - Octubre - Tensión Arterial</h2>
...
onMounted(() => {
  if ((formDataStore.formDataControlPrenatal as any).octubreTia) {
    tensionArterialCompleta.value = (formDataStore.formDataControlPrenatal as any).octubreTia
    // Extraer valores individuales si es necesario
    const partes = (formDataStore.formDataControlPrenatal as any).octubreTia.split('/')
    if (partes.length === 2) {
...
watch([tensionArterialCompleta], (newValue) => {
  if (newValue && !mensajeErrorRelacion.value) {
    (formDataStore.formDataControlPrenatal as any).octubreTia = newValue
  }
}, { deep: true })
