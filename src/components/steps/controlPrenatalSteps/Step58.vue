...
onMounted(() => {
  // Verificar si formDataControlPrenatal.agostoSdg tiene un valor y establecerlo
  if (formDataControlPrenatal.agostoSdg) {
    agostoSdg.value = formDataControlPrenatal.agostoSdg.toString();
  }
});

onUnmounted(() => {
  // Asegurar que formData tenga un valor inicial para agostoSdg
  if (!formDataControlPrenatal.agostoSdg) {
    formDataControlPrenatal.agostoSdg = agostoSdg.value ? parseInt(agostoSdg.value) : undefined;
  }
});

// Sincronizar agostoSdg con formData
watch(agostoSdg, (newValue) => {
  if (newValue && newValue !== '') {
    formDataControlPrenatal.agostoSdg = parseInt(newValue);
  } else {
    formDataControlPrenatal.agostoSdg = undefined;
  }
});

// Computed para la categoría de gestación
const categoriaGestacionComputed = computed(() => {
  return determinarCategoriaGestacion(agostoSdg.value);
});
...
const mensajeErrorSemana = computed(() => {
  if (!agostoSdg.value || agostoSdg.value === '') return '';
  
  const semana = parseInt(agostoSdg.value);
...
const seleccionarPreset = (valor) => {
  agostoSdg.value = valor.toString();
};
...
<template>
  <div>
      ...
      <div class="mb-4">
        ...
        <div class="mb-4 mt-4">
          <div class="grid grid-cols-4 gap-3">
            <button
              ...
              :class="agostoSdg === preset.valor.toString() 
        ...
        <div class="mb-4">
          <div class="flex items-center justify-center gap-3">
            <label for="agostoSdg" class="font-medium text-gray-700">Personalizar:</label>
            <input
              id="agostoSdg"
              v-model="agostoSdg"
              type="number"
    ...
    <div class="mb-6">
      <div v-if="agostoSdg && !mensajeErrorSemana" class="mb-4">
      ...
      <div v-else-if="agostoSdg && mensajeErrorSemana" class="p-3 bg-red-50 border border-red-200 rounded-lg">
