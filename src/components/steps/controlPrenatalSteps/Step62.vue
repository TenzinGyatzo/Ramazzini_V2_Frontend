...
onMounted(() => {
  // Verificar si formDataControlPrenatal tiene valores para septiembre y establecerlos
  if (formDataControlPrenatal.septiembreTia) {
    // Extraer sistólica y diastólica del formato "120/80"
    const tensionCompleta = formDataControlPrenatal.septiembreTia;
...
onUnmounted(() => {
  // Guardar la tensión arterial completa en el campo de septiembre
  if (tensionArterialCompleta.value) {
    formDataControlPrenatal.septiembreTia = tensionArterialCompleta.value;
  } else {
    formDataControlPrenatal.septiembreTia = null;
  }
});
...
watch([tensionArterialSistolica, tensionArterialDiastolica], ([sistolica, diastolica]) => {
  ...
  // Guardar en el campo de septiembre cuando se complete
  if (tensionCompleta) {
    formDataControlPrenatal.septiembreTia = tensionCompleta;
  } else {
    formDataControlPrenatal.septiembreTia = null;
  }
});
