...
onMounted(() => {
  // Verificar si formDataControlPrenatal.febreroSdg tiene un valor y establecerlo
  if (formDataControlPrenatal.febreroSdg) {
    febreroSdg.value = formDataControlPrenatal.febreroSdg.toString();
  }
});

onUnmounted(() => {
  // Asegurar que formData tenga un valor inicial para febreroSdg
  if (!formDataControlPrenatal.febreroSdg) {
    formDataControlPrenatal.febreroSdg = febreroSdg.value ? parseInt(febreroSdg.value) : undefined;
  }
});

// Sincronizar febreroSdg con formData
watch(febreroSdg, (newValue) => {
  if (newValue && newValue !== '') {
    formDataControlPrenatal.febreroSdg = parseInt(newValue);
  } else {
    formDataControlPrenatal.febreroSdg = undefined;
  }
});

// Computed para la categoría de gestación
const categoriaGestacionComputed = computed(() => {
  return determinarCategoriaGestacion(febreroSdg.value);
});
...
const mensajeErrorSemana = computed(() => {
  if (!febreroSdg.value || febreroSdg.value === '') return '';
  
  const semana = parseInt(febreroSdg.value);
...
const seleccionarPreset = (valor) => {
  febreroSdg.value = valor.toString();
};
...
<template>
  <div>
      ...
      <div class="mb-4">
        ...
        <div class="mb-4 mt-4">
          <div class="grid grid-cols-4 gap-3">
            <button
              ...
              :class="febreroSdg === preset.valor.toString() 
        ...
        <div class="mb-4">
          <div class="flex items-center justify-center gap-3">
            <label for="febreroSdg" class="font-medium text-gray-700">Personalizar:</label>
            <input
              id="febreroSdg"
              v-model="febreroSdg"
              type="number"
    ...
    <div class="mb-6">
      <div v-if="febreroSdg && !mensajeErrorSemana" class="mb-4">
      ...
      <div v-else-if="febreroSdg && mensajeErrorSemana" class="p-3 bg-red-50 border border-red-200 rounded-lg">
