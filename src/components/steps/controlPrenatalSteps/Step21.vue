...
onMounted(() => {
  // Verificar si formDataControlPrenatal.febreroFcf tiene un valor y establecerlo
  if (formDataControlPrenatal.febreroFcf) {
    febreroFcf.value = formDataControlPrenatal.febreroFcf.toString();
  }
});

onUnmounted(() => {
  // Asegurar que formData tenga un valor inicial para febreroFcf
  if (!formDataControlPrenatal.febreroFcf) {
    formDataControlPrenatal.febreroFcf = febreroFcf.value ? parseInt(febreroFcf.value) : undefined;
  }
});

// Sincronizar febreroFcf con formData
watch(febreroFcf, (newValue) => {
  if (newValue && newValue !== '') {
    formDataControlPrenatal.febreroFcf = parseInt(newValue);
  } else {
    formDataControlPrenatal.febreroFcf = undefined;
  }
});

// Computed para la categoría de FCF
const categoriaFcfComputed = computed(() => {
  return determinarCategoriaFcf(febreroFcf.value);
});

// Validaciones reactivas
const mensajeErrorFcf = computed(() => {
  if (!febreroFcf.value || febreroFcf.value === '') return '';
  
  const fcf = parseInt(febreroFcf.value);
...
const seleccionarPreset = (valor) => {
  febreroFcf.value = valor.toString();
};
...
<template>
  <div>
      ...
      <div class="mb-4">
        ...
        <div class="mb-4 mt-4">
          <div class="grid grid-cols-5 gap-3">
            <button
              ...
              :class="febreroFcf === preset.valor.toString() 
        ...
        <div class="mb-4">
          <div class="flex items-center justify-center gap-3">
            <label for="febreroFcf" class="font-medium text-gray-700">Personalizar:</label>
            <input
              id="febreroFcf"
              v-model="febreroFcf"
              type="number"
    ...
    <div class="mb-6">
      ...
      <div v-else-if="febreroFcf && mensajeErrorFcf" class="p-3 bg-red-50 border border-red-200 rounded-lg">
