<template>
  <div class="step-container">
    <h2 class="step-title">Control Prenatal - Febrero - Frecuencia Cardíaca Fetal</h2>
    
    <div class="form-section">
      <div class="input-group">
        <label for="febreroFcf" class="form-label">Frecuencia Cardíaca Fetal (FCF) - Febrero</label>
...
onMounted(() => {
  if ((formDataStore.formDataControlPrenatal as any).febreroFcf) {
    febreroFcf.value = (formDataStore.formDataControlPrenatal as any).febreroFcf
    categoriaFcf.value = determinarCategoriaFcf(Number((formDataStore.formDataControlPrenatal as any).febreroFcf))
  }
})
...
watch([febreroFcf], (newValue) => {
  if (newValue && !mensajeErrorFcf.value) {
    (formDataStore.formDataControlPrenatal as any).febreroFcf = newValue
...
const categoriaFcfComputed = computed(() => {
  const fcf = Number(febreroFcf.value)
  if (!febreroFcf.value) return { text: 'No especificado', class: 'text-gray-500' }
...
const mensajeErrorFcf = computed(() => {
  if (!febreroFcf.value) return 'La FCF es requerida'
  const fcf = Number(febreroFcf.value)
