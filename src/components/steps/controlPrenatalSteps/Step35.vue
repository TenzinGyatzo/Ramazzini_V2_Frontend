...
onMounted(() => {
  // Verificar si formDataControlPrenatal.abrilFondoUterino tiene un valor y establecerlo
  if (formDataControlPrenatal.abrilFondoUterino) {
    abrilFondoUterino.value = formDataControlPrenatal.abrilFondoUterino.toString();
  }
});

onUnmounted(() => {
  // Asegurar que formData tenga un valor inicial para abrilFondoUterino
  if (!formDataControlPrenatal.abrilFondoUterino) {
    formDataControlPrenatal.abrilFondoUterino = abrilFondoUterino.value ? parseFloat(abrilFondoUterino.value) : undefined;
  }
});

// Sincronizar abrilFondoUterino con formData
watch(abrilFondoUterino, (newValue) => {
  if (newValue && newValue !== '') {
    formDataControlPrenatal.abrilFondoUterino = parseFloat(newValue);
  } else {
    formDataControlPrenatal.abrilFondoUterino = undefined;
  }
});

// Computed para la categoría del fondo uterino
const categoriaFondoUterinoComputed = computed(() => {
  return determinarCategoriaFondoUterino(abrilFondoUterino.value, formDataControlPrenatal.abrilSdg);
});

// Computed para el fondo uterino esperado
const fondoUterinoEsperadoComputed = computed(() => {
  return calcularFondoUterinoEsperado(formDataControlPrenatal.abrilSdg);
});

// Validaciones reactivas
const mensajeErrorFondoUterino = computed(() => {
  if (!abrilFondoUterino.value || abrilFondoUterino.value === '') return '';
  
  const fondoUterino = parseFloat(abrilFondoUterino.value);
...
const seleccionarPreset = (valor) => {
  abrilFondoUterino.value = valor.toString();
};
...
<template>
  <div>
      ...
      <div class="mb-4">
        ...
        <div class="mb-4 mt-4">
          <div class="grid grid-cols-3 gap-3">
            <button
              ...
              :class="abrilFondoUterino === preset.valor.toString() 
        ...
        <div class="mb-4">
          <div class="flex items-center justify-center gap-3">
            <label for="abrilFondoUterino" class="font-medium text-gray-700">Personalizar:</label>
            <input
              id="abrilFondoUterino"
              v-model="abrilFondoUterino"
              type="number"
              min="0"
              max="50"
              step="0.5"
    ...
    <div class="mb-4">
      ...
      <div v-else-if="abrilFondoUterino && mensajeErrorFondoUterino" class="p-3 bg-red-50 border border-red-200 rounded-lg">
    ...
    <div v-if="fondoUterinoEsperadoComputed && formDataControlPrenatal.abrilSdg" class="mb-2">
