...
onMounted(() => {
  // Verificar si formDataControlPrenatal.marzoFondoUterino tiene un valor y establecerlo
  if (formDataControlPrenatal.marzoFondoUterino) {
    marzoFondoUterino.value = formDataControlPrenatal.marzoFondoUterino.toString();
  }
});

onUnmounted(() => {
  // Asegurar que formData tenga un valor inicial para marzoFondoUterino
  if (!formDataControlPrenatal.marzoFondoUterino) {
    formDataControlPrenatal.marzoFondoUterino = marzoFondoUterino.value ? parseFloat(marzoFondoUterino.value) : undefined;
  }
});

// Sincronizar marzoFondoUterino con formData
watch(marzoFondoUterino, (newValue) => {
  if (newValue && newValue !== '') {
    formDataControlPrenatal.marzoFondoUterino = parseFloat(newValue);
  } else {
    formDataControlPrenatal.marzoFondoUterino = undefined;
  }
});

// Computed para la categoría del fondo uterino
const categoriaFondoUterinoComputed = computed(() => {
  return determinarCategoriaFondoUterino(marzoFondoUterino.value, formDataControlPrenatal.marzoSdg);
});

// Computed para el fondo uterino esperado
const fondoUterinoEsperadoComputed = computed(() => {
  return calcularFondoUterinoEsperado(formDataControlPrenatal.marzoSdg);
});

// Validaciones reactivas
const mensajeErrorFondoUterino = computed(() => {
  if (!marzoFondoUterino.value || marzoFondoUterino.value === '') return '';
  
  const fondoUterino = parseFloat(marzoFondoUterino.value);
...
const seleccionarPreset = (valor) => {
  marzoFondoUterino.value = valor.toString();
};
...
<template>
  <div>
      ...
      <div class="mb-4">
        ...
        <div class="mb-4 mt-4">
          <div class="grid grid-cols-3 gap-3">
            <button
              ...
              :class="marzoFondoUterino === preset.valor.toString() 
        ...
        <div class="mb-4">
          <div class="flex items-center justify-center gap-3">
            <label for="marzoFondoUterino" class="font-medium text-gray-700">Personalizar:</label>
            <input
              id="marzoFondoUterino"
              v-model="marzoFondoUterino"
              type="number"
              min="0"
              max="50"
              step="0.5"
    ...
    <div class="mb-4">
      ...
      <div v-else-if="marzoFondoUterino && mensajeErrorFondoUterino" class="p-3 bg-red-50 border border-red-200 rounded-lg">
    ...
    <div v-if="fondoUterinoEsperadoComputed && formDataControlPrenatal.marzoSdg" class="mb-2">
