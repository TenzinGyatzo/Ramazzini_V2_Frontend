...
onMounted(() => {
  // Verificar si formDataControlPrenatal.septiembreFcf tiene un valor y establecerlo
  if (formDataControlPrenatal.septiembreFcf) {
    septiembreFcf.value = formDataControlPrenatal.septiembreFcf.toString();
  }
});
...
const categoriaFcfComputed = computed(() => {
  return determinarCategoriaFcf(septiembreFcf.value);
});

// Validaciones reactivas
const mensajeErrorFcf = computed(() => {
  if (!septiembreFcf.value || septiembreFcf.value === '') return '';
  
  const fcf = parseInt(septiembreFcf.value);
...
const seleccionarPreset = (valor) => {
  septiembreFcf.value = valor.toString();
};
...
<template>
  <div>
    ...
    <div class="mb-4">
      ...
      <div class="mb-4 mt-4">
        <div class="grid grid-cols-5 gap-3">
          <button
            ...
            :class="septiembreFcf === preset.valor.toString() 
      ...
      <div class="mb-4">
        <div class="flex items-center justify-center gap-3">
          <label for="septiembreFcf" class="font-medium text-gray-700">Personalizar:</label>
          <input
            id="septiembreFcf"
            v-model="septiembreFcf"
            type="number"
            min="60"
            max="250"
            placeholder="Ej: 135"
    ...
    <div class="mb-6">
      ...
      <div v-else-if="septiembreFcf && mensajeErrorFcf" class="p-3 bg-red-50 border border-red-200 rounded-lg">
