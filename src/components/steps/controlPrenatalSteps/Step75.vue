<template>
  <div class="step-container">
    <h2 class="step-title">Control Prenatal - Noviembre - Frecuencia Cardíaca Fetal</h2>
    
    <div class="form-section">
      <div class="input-group">
        <label for="noviembreFcf" class="form-label">Frecuencia Cardíaca Fetal (FCF) - Noviembre</label>
...
onMounted(() => {
  ...
  if (savedData.noviembreFcf) {
    noviembreFcf.value = savedData.noviembreFcf
    categoriaFcf.value = determinarCategoriaFcf(Number(savedData.noviembreFcf))
  }
})
...
const categoriaFcfComputed = computed(() => {
  const fcf = Number(noviembreFcf.value)
  if (!noviembreFcf.value) return { text: 'No especificado', class: 'text-gray-500' }
...
const mensajeErrorFcf = computed(() => {
  if (!noviembreFcf.value) return 'La FCF es requerida'
  const fcf = Number(noviembreFcf.value)
