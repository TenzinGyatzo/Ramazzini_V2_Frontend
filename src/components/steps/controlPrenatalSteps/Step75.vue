...
onMounted(() => {
  // Verificar si formDataControlPrenatal.noviembreFcf tiene un valor y establecerlo
  if (formDataControlPrenatal.noviembreFcf) {
    noviembreFcf.value = formDataControlPrenatal.noviembreFcf.toString();
  }
});

onUnmounted(() => {
  // Asegurar que formData tenga un valor inicial para noviembreFcf
  if (!formDataControlPrenatal.noviembreFcf) {
    formDataControlPrenatal.noviembreFcf = noviembreFcf.value ? parseInt(noviembreFcf.value) : undefined;
  }
});

// Sincronizar noviembreFcf con formData
watch(noviembreFcf, (newValue) => {
  if (newValue && newValue !== '') {
    formDataControlPrenatal.noviembreFcf = parseInt(newValue);
  } else {
    formDataControlPrenatal.noviembreFcf = undefined;
  }
});

// Computed para la categoría de FCF
const categoriaFcfComputed = computed(() => {
  return determinarCategoriaFcf(noviembreFcf.value);
});

// Validaciones reactivas
const mensajeErrorFcf = computed(() => {
  if (!noviembreFcf.value || noviembreFcf.value === '') return '';
  
  const fcf = parseInt(noviembreFcf.value);
...
const seleccionarPreset = (valor) => {
  noviembreFcf.value = valor.toString();
};
...
<template>
  <div>
      ...
      <div class="mb-4">
        ...
        <div class="mb-4 mt-4">
          <div class="grid grid-cols-5 gap-3">
            <button
              ...
              :class="noviembreFcf === preset.valor.toString() 
        ...
        <div class="mb-4">
          <div class="flex items-center justify-center gap-3">
            <label for="noviembreFcf" class="font-medium text-gray-700">Personalizar:</label>
            <input
              id="noviembreFcf"
              v-model="noviembreFcf"
              type="number"
              min="60"
              max="250"
              placeholder="Ej: 135"
    ...
    <div class="mb-6">
      ...
      <div v-else-if="noviembreFcf && mensajeErrorFcf" class="p-3 bg-red-50 border border-red-200 rounded-lg">
