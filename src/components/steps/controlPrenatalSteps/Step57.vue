...
onMounted(() => {
  // Verificar si formDataControlPrenatal.agostoFcf tiene un valor y establecerlo
  if (formDataControlPrenatal.agostoFcf) {
    agostoFcf.value = formDataControlPrenatal.agostoFcf.toString();
  }
});

onUnmounted(() => {
  // Asegurar que formData tenga un valor inicial para agostoFcf
  if (!formDataControlPrenatal.agostoFcf) {
    formDataControlPrenatal.agostoFcf = agostoFcf.value ? parseInt(agostoFcf.value) : undefined;
  }
});

// Sincronizar agostoFcf con formData
watch(agostoFcf, (newValue) => {
  if (newValue && newValue !== '') {
    formDataControlPrenatal.agostoFcf = parseInt(newValue);
  } else {
    formDataControlPrenatal.agostoFcf = undefined;
  }
});

// Computed para la categoría de FCF
const categoriaFcfComputed = computed(() => {
  return determinarCategoriaFcf(agostoFcf.value);
});

// Validaciones reactivas
const mensajeErrorFcf = computed(() => {
  if (!agostoFcf.value || agostoFcf.value === '') return '';
  
  const fcf = parseInt(agostoFcf.value);
...
const seleccionarPreset = (valor) => {
  agostoFcf.value = valor.toString();
};
...
<template>
  <div>
      ...
      <div class="mb-4">
        ...
        <div class="mb-4 mt-4">
          <div class="grid grid-cols-5 gap-3">
            <button
              ...
              :class="agostoFcf === preset.valor.toString() 
        ...
        <div class="mb-4">
          <div class="flex items-center justify-center gap-3">
            <label for="agostoFcf" class="font-medium text-gray-700">Personalizar:</label>
            <input
              id="agostoFcf"
              v-model="agostoFcf"
              type="number"
    ...
    <div class="mb-6">
      ...
      <div v-else-if="agostoFcf && mensajeErrorFcf" class="p-3 bg-red-50 border border-red-200 rounded-lg">
