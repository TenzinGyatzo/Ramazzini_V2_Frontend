<template>
  <div class="step-container">
    <h2 class="step-title">Control Prenatal - Agosto - Frecuencia Cardíaca Fetal</h2>
    
    <div class="form-section">
      <div class="input-group">
        <label for="agostoFcf" class="form-label">Frecuencia Cardíaca Fetal (FCF) - Agosto</label>
...
onMounted(() => {
  if ((formDataStore.formDataControlPrenatal as any).agostoFcf) {
    agostoFcf.value = (formDataStore.formDataControlPrenatal as any).agostoFcf
    categoriaFcf.value = determinarCategoriaFcf(Number((formDataStore.formDataControlPrenatal as any).agostoFcf))
  }
})
...
watch([agostoFcf], (newValue) => {
  if (newValue && !mensajeErrorFcf.value) {
    (formDataStore.formDataControlPrenatal as any).agostoFcf = newValue
...
const categoriaFcfComputed = computed(() => {
  const fcf = Number(agostoFcf.value)
  if (!agostoFcf.value) return { text: 'No especificado', class: 'text-gray-500' }
...
const mensajeErrorFcf = computed(() => {
  if (!agostoFcf.value) return 'La FCF es requerida'
  const fcf = Number(agostoFcf.value)
