<template>
  <div class="step-container">
    <h2 class="step-title">Control Prenatal - Junio - Frecuencia Cardíaca Fetal</h2>
    
    <div class="form-section">
      <div class="input-group">
        <label for="junioFcf" class="form-label">Frecuencia Cardíaca Fetal (FCF) - Junio</label>
...
onMounted(() => {
  if ((formDataStore.formDataControlPrenatal as any).junioFcf) {
    junioFcf.value = (formDataStore.formDataControlPrenatal as any).junioFcf
    categoriaFcf.value = determinarCategoriaFcf(Number((formDataStore.formDataControlPrenatal as any).junioFcf))
  }
})
...
watch([junioFcf], (newValue) => {
  if (newValue && !mensajeErrorFcf.value) {
    (formDataStore.formDataControlPrenatal as any).junioFcf = newValue
...
const categoriaFcfComputed = computed(() => {
  const fcf = Number(junioFcf.value)
  if (!junioFcf.value) return { text: 'No especificado', class: 'text-gray-500' }
...
const mensajeErrorFcf = computed(() => {
  if (!junioFcf.value) return 'La FCF es requerida'
  const fcf = Number(junioFcf.value)
