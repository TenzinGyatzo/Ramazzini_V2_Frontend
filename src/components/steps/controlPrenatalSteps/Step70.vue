...
onMounted(() => {
  // Verificar si formDataControlPrenatal.octubreSdg tiene un valor y establecerlo
  if (formDataControlPrenatal.octubreSdg) {
    octubreSdg.value = formDataControlPrenatal.octubreSdg.toString();
  }
});
...
const categoriaGestacionComputed = computed(() => {
  return determinarCategoriaGestacion(octubreSdg.value);
});
...
const mensajeErrorSemana = computed(() => {
  if (!octubreSdg.value || octubreSdg.value === '') return '';
  
  const semana = parseInt(octubreSdg.value);
...
const seleccionarPreset = (valor) => {
  octubreSdg.value = valor.toString();
};
...
<template>
  <div>
      ...
      <div class="mb-4">
        ...
        <div class="mb-4 mt-4">
          <div class="grid grid-cols-4 gap-3">
            <button
              ...
              :class="octubreSdg === preset.valor.toString() 
        ...
        <div class="mb-4">
          <div class="flex items-center justify-center gap-3">
            <label for="octubreSdg" class="font-medium text-gray-700">Personalizar:</label>
            <input
              id="octubreSdg"
              v-model="octubreSdg"
              type="number"
    ...
    <div class="mb-6">
      <div v-if="octubreSdg && !mensajeErrorSemana" class="mb-4">
      ...
      <div v-else-if="octubreSdg && mensajeErrorSemana" class="p-3 bg-red-50 border border-red-200 rounded-lg">
