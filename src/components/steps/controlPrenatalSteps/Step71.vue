...
onMounted(() => {
  // Verificar si formDataControlPrenatal.octubreFondoUterino tiene un valor y establecerlo
  if (formDataControlPrenatal.octubreFondoUterino) {
    octubreFondoUterino.value = formDataControlPrenatal.octubreFondoUterino.toString();
  }
});
...
const categoriaFondoUterinoComputed = computed(() => {
  return determinarCategoriaFondoUterino(octubreFondoUterino.value, formDataControlPrenatal.octubreSdg);
});

// Computed para el fondo uterino esperado
const fondoUterinoEsperadoComputed = computed(() => {
  return calcularFondoUterinoEsperado(formDataControlPrenatal.octubreSdg);
});

// Validaciones reactivas
const mensajeErrorFondoUterino = computed(() => {
  if (!octubreFondoUterino.value || octubreFondoUterino.value === '') return '';
  
  const fondoUterino = parseFloat(octubreFondoUterino.value);
...
const seleccionarPreset = (valor) => {
  octubreFondoUterino.value = valor.toString();
};
...
<template>
  <div>
    ...
    <div class="mb-4">
      ...
      <div class="mb-4 mt-4">
        <div class="grid grid-cols-3 gap-3">
          <button
            ...
            :class="octubreFondoUterino === preset.valor.toString() 
      ...
      <div class="mb-4">
        <div class="flex items-center justify-center gap-3">
          <label for="octubreFondoUterino" class="font-medium text-gray-700">Personalizar:</label>
          <input
            id="octubreFondoUterino"
            v-model="octubreFondoUterino"
            type="number"
            min="0"
            max="50"
            step="0.5"
    ...
    <div class="mb-4">
      ...
      <div v-else-if="octubreFondoUterino && mensajeErrorFondoUterino" class="p-3 bg-red-50 border border-red-200 rounded-lg">
    ...
    <div v-if="fondoUterinoEsperadoComputed && formDataControlPrenatal.octubreSdg" class="mb-2">
