<template>
  <div class="step-container">
    <h2 class="step-title">Control Prenatal - Abril - Tensión Arterial</h2>
...
onMounted(() => {
  if ((formDataStore.formDataControlPrenatal as any).abrilTia) {
    tensionArterialCompleta.value = (formDataStore.formDataControlPrenatal as any).abrilTia
    // Extraer valores individuales si es necesario
    const partes = (formDataStore.formDataControlPrenatal as any).abrilTia.split('/')
    if (partes.length === 2) {
...
watch([tensionArterialCompleta], (newValue) => {
  if (newValue && !mensajeErrorRelacion.value) {
    (formDataStore.formDataControlPrenatal as any).abrilTia = newValue
  }
}, { deep: true })
