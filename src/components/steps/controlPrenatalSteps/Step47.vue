...
onMounted(() => {
  // Verificar si formDataControlPrenatal.junioFondoUterino tiene un valor y establecerlo
  if (formDataControlPrenatal.junioFondoUterino) {
    junioFondoUterino.value = formDataControlPrenatal.junioFondoUterino.toString();
  }
});

onUnmounted(() => {
  // Asegurar que formData tenga un valor inicial para junioFondoUterino
  if (!formDataControlPrenatal.junioFondoUterino) {
    formDataControlPrenatal.junioFondoUterino = junioFondoUterino.value ? parseFloat(junioFondoUterino.value) : undefined;
  }
});

// Sincronizar junioFondoUterino con formData
watch(junioFondoUterino, (newValue) => {
  if (newValue && newValue !== '') {
    formDataControlPrenatal.junioFondoUterino = parseFloat(newValue);
  } else {
    formDataControlPrenatal.junioFondoUterino = undefined;
  }
});

// Computed para la categoría del fondo uterino
const categoriaFondoUterinoComputed = computed(() => {
  return determinarCategoriaFondoUterino(junioFondoUterino.value, formDataControlPrenatal.junioSdg);
});

// Computed para el fondo uterino esperado
const fondoUterinoEsperadoComputed = computed(() => {
  return calcularFondoUterinoEsperado(formDataControlPrenatal.junioSdg);
});

// Validaciones reactivas
const mensajeErrorFondoUterino = computed(() => {
  if (!junioFondoUterino.value || junioFondoUterino.value === '') return '';
  
  const fondoUterino = parseFloat(junioFondoUterino.value);
...
const seleccionarPreset = (valor) => {
  junioFondoUterino.value = valor.toString();
};
...
<template>
  <div>
      ...
      <div class="mb-4">
        ...
        <div class="mb-4 mt-4">
          <div class="grid grid-cols-3 gap-3">
            <button
              ...
              :class="junioFondoUterino === preset.valor.toString() 
        ...
        <div class="mb-4">
          <div class="flex items-center justify-center gap-3">
            <label for="junioFondoUterino" class="font-medium text-gray-700">Personalizar:</label>
            <input
              id="junioFondoUterino"
              v-model="junioFondoUterino"
              type="number"
              min="0"
              max="50"
              step="0.5"
    ...
    <div class="mb-4">
      ...
      <div v-else-if="junioFondoUterino && mensajeErrorFondoUterino" class="p-3 bg-red-50 border border-red-200 rounded-lg">
    ...
    <div v-if="fondoUterinoEsperadoComputed && formDataControlPrenatal.junioSdg" class="mb-2">
