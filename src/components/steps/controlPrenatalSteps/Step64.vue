...
onMounted(() => {
  // Verificar si formDataControlPrenatal.septiembreSdg tiene un valor y establecerlo
  if (formDataControlPrenatal.septiembreSdg) {
    septiembreSdg.value = formDataControlPrenatal.septiembreSdg.toString();
  }
});

onUnmounted(() => {
  // Asegurar que formData tenga un valor inicial para septiembreSdg
  if (!formDataControlPrenatal.septiembreSdg) {
    formDataControlPrenatal.septiembreSdg = septiembreSdg.value ? parseInt(septiembreSdg.value) : undefined;
  }
});

// Sincronizar septiembreSdg con formData
watch(septiembreSdg, (newValue) => {
  if (newValue && newValue !== '') {
    formDataControlPrenatal.septiembreSdg = parseInt(newValue);
  } else {
    formDataControlPrenatal.septiembreSdg = undefined;
  }
});

// Computed para la categoría de gestación
const categoriaGestacionComputed = computed(() => {
  return determinarCategoriaGestacion(septiembreSdg.value);
});
...
const mensajeErrorSemana = computed(() => {
  if (!septiembreSdg.value || septiembreSdg.value === '') return '';
  
  const semana = parseInt(septiembreSdg.value);
...
const seleccionarPreset = (valor) => {
  septiembreSdg.value = valor.toString();
};
...
<template>
  <div>
      ...
      <div class="mb-4">
        ...
        <div class="mb-4 mt-4">
          <div class="grid grid-cols-4 gap-3">
            <button
              ...
              :class="septiembreSdg === preset.valor.toString() 
        ...
        <div class="mb-4">
          <div class="flex items-center justify-center gap-3">
            <label for="septiembreSdg" class="font-medium text-gray-700">Personalizar:</label>
            <input
              id="septiembreSdg"
              v-model="septiembreSdg"
              type="number"
              min="1"
              max="45"
              placeholder="Ej: 25"
    ...
    <div class="mb-6">
      ...
      <div v-else-if="septiembreSdg && mensajeErrorSemana" class="p-3 bg-red-50 border border-red-200 rounded-lg">
