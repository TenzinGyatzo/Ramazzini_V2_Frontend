...
onMounted(() => {
  // Verificar si formDataControlPrenatal.agostoFondoUterino tiene un valor y establecerlo
  if (formDataControlPrenatal.agostoFondoUterino) {
    agostoFondoUterino.value = formDataControlPrenatal.agostoFondoUterino.toString();
  }
});

onUnmounted(() => {
  // Asegurar que formData tenga un valor inicial para agostoFondoUterino
  if (!formDataControlPrenatal.agostoFondoUterino) {
    formDataControlPrenatal.agostoFondoUterino = agostoFondoUterino.value ? parseFloat(agostoFondoUterino.value) : undefined;
  }
});

// Sincronizar agostoFondoUterino con formData
watch(agostoFondoUterino, (newValue) => {
  if (newValue && newValue !== '') {
    formDataControlPrenatal.agostoFondoUterino = parseFloat(newValue);
  } else {
    formDataControlPrenatal.agostoFondoUterino = undefined;
  }
});

// Computed para la categoría del fondo uterino
const categoriaFondoUterinoComputed = computed(() => {
  return determinarCategoriaFondoUterino(agostoFondoUterino.value, formDataControlPrenatal.agostoSdg);
});

// Computed para el fondo uterino esperado
const fondoUterinoEsperadoComputed = computed(() => {
  return calcularFondoUterinoEsperado(formDataControlPrenatal.agostoSdg);
});

// Validaciones reactivas
const mensajeErrorFondoUterino = computed(() => {
  if (!agostoFondoUterino.value || agostoFondoUterino.value === '') return '';
  
  const fondoUterino = parseFloat(agostoFondoUterino.value);
...
const seleccionarPreset = (valor) => {
  agostoFondoUterino.value = valor.toString();
};
...
<template>
  <div>
      ...
      <div class="mb-4">
        ...
        <div class="mb-4 mt-4">
          <div class="grid grid-cols-3 gap-3">
            <button
              ...
              :class="agostoFondoUterino === preset.valor.toString() 
        ...
        <div class="mb-4">
          <div class="flex items-center justify-center gap-3">
            <label for="agostoFondoUterino" class="font-medium text-gray-700">Personalizar:</label>
            <input
              id="agostoFondoUterino"
              v-model="agostoFondoUterino"
              type="number"
              min="0"
              max="50"
              step="0.5"
    ...
    <div class="mb-4">
      ...
      <div v-else-if="agostoFondoUterino && mensajeErrorFondoUterino" class="p-3 bg-red-50 border border-red-200 rounded-lg">
    ...
    <div v-if="fondoUterinoEsperadoComputed && formDataControlPrenatal.agostoSdg" class="mb-2">
