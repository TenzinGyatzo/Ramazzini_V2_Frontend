...
onMounted(() => {
  ...
  if (formDataControlPrenatal.eneroPeso) {
    eneroPeso.value = formDataControlPrenatal.eneroPeso;
  }
  if (formDataControlPrenatal.eneroImc) {
    eneroImc.value = formDataControlPrenatal.eneroImc;
  }
...
const puedeCalcularIMC = computed(() => {
  return eneroPeso.value && alturaDisponible.value;
});

// Validaciones reactivas similares a Step2.vue
const mensajeErrorPeso = computed(() => {
  if (!eneroPeso.value || eneroPeso.value === '') return '';
  
  const peso = parseFloat(eneroPeso.value);
    ...
    <div class="mb-6">
      ...
      <div class="font-light mt-2">
        <input type="number"
            ...
            :disabled="!eneroPeso || !alturaDisponible"
            readonly>
      </div>
