...
onMounted(() => {
  // Verificar si formDataControlPrenatal.febreroFondoUterino tiene un valor y establecerlo
  if (formDataControlPrenatal.febreroFondoUterino) {
    febreroFondoUterino.value = formDataControlPrenatal.febreroFondoUterino.toString();
  }
});

onUnmounted(() => {
  // Asegurar que formData tenga un valor inicial para febreroFondoUterino
  if (!formDataControlPrenatal.febreroFondoUterino) {
    formDataControlPrenatal.febreroFondoUterino = febreroFondoUterino.value ? parseFloat(febreroFondoUterino.value) : undefined;
  }
});

// Sincronizar febreroFondoUterino con formData
watch(febreroFondoUterino, (newValue) => {
  if (newValue && newValue !== '') {
    formDataControlPrenatal.febreroFondoUterino = parseFloat(newValue);
  } else {
    formDataControlPrenatal.febreroFondoUterino = undefined;
  }
});

// Computed para la categoría del fondo uterino
const categoriaFondoUterinoComputed = computed(() => {
  return determinarCategoriaFondoUterino(febreroFondoUterino.value, formDataControlPrenatal.febreroSdg);
});

// Computed para el fondo uterino esperado
const fondoUterinoEsperadoComputed = computed(() => {
  return calcularFondoUterinoEsperado(formDataControlPrenatal.febreroSdg);
});

// Validaciones reactivas
const mensajeErrorFondoUterino = computed(() => {
  if (!febreroFondoUterino.value || febreroFondoUterino.value === '') return '';
  
  const fondoUterino = parseFloat(febreroFondoUterino.value);
...
const seleccionarPreset = (valor) => {
  febreroFondoUterino.value = valor.toString();
};
...
<template>
  <div>
      ...
      <div class="mb-4">
        ...
        <div class="mb-4 mt-4">
          <div class="grid grid-cols-3 gap-3">
            <button
              ...
              :class="febreroFondoUterino === preset.valor.toString() 
        ...
        <div class="mb-4">
          <div class="flex items-center justify-center gap-3">
            <label for="febreroFondoUterino" class="font-medium text-gray-700">Personalizar:</label>
            <input
              id="febreroFondoUterino"
              v-model="febreroFondoUterino"
              type="number"
              min="0"
              max="50"
              step="0.5"
    ...
    <div class="mb-4">
      ...
      <div v-else-if="febreroFondoUterino && mensajeErrorFondoUterino" class="p-3 bg-red-50 border border-red-200 rounded-lg">
    ...
    <div v-if="fondoUterinoEsperadoComputed && formDataControlPrenatal.febreroSdg" class="mb-2">
