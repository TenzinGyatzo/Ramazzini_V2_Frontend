<template>
  <div class="step-container">
    <h2 class="step-title">Control Prenatal - Diciembre - Tensión Arterial</h2>
...
onMounted(() => {
  if ((formDataStore.formDataControlPrenatal as any).diciembreTia) {
    tensionArterialCompleta.value = (formDataStore.formDataControlPrenatal as any).diciembreTia
    // Extraer valores individuales si es necesario
    const partes = (formDataStore.formDataControlPrenatal as any).diciembreTia.split('/')
    if (partes.length === 2) {
...
watch([tensionArterialCompleta], (newValue) => {
  if (newValue && !mensajeErrorRelacion.value) {
    (formDataStore.formDataControlPrenatal as any).diciembreTia = newValue
  }
}, { deep: true })
