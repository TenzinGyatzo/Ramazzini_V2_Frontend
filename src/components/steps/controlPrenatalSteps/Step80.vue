...
onMounted(() => {
  // Verificar si formDataControlPrenatal tiene valores para diciembre y establecerlos
  if (formDataControlPrenatal.diciembreTia) {
    // Extraer sistólica y diastólica del formato "120/80"
    const tensionCompleta = formDataControlPrenatal.diciembreTia;
...
onUnmounted(() => {
  // Guardar la tensión arterial completa en el campo de diciembre
  if (tensionArterialCompleta.value) {
    formDataControlPrenatal.diciembreTia = tensionArterialCompleta.value;
  } else {
    formDataControlPrenatal.diciembreTia = null;
  }
});
...
watch([tensionArterialSistolica, tensionArterialDiastolica], ([sistolica, diastolica]) => {
  ...
  // Guardar en el campo de diciembre cuando se complete
  if (tensionCompleta) {
    formDataControlPrenatal.diciembreTia = tensionCompleta;
  } else {
    formDataControlPrenatal.diciembreTia = null;
  }
});
