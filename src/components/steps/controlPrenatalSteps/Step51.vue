...
onMounted(() => {
  // Verificar si formDataControlPrenatal.julioFcf tiene un valor y establecerlo
  if (formDataControlPrenatal.julioFcf) {
    julioFcf.value = formDataControlPrenatal.julioFcf.toString();
  }
});

onUnmounted(() => {
  // Asegurar que formData tenga un valor inicial para julioFcf
  if (!formDataControlPrenatal.julioFcf) {
    formDataControlPrenatal.julioFcf = julioFcf.value ? parseInt(julioFcf.value) : undefined;
  }
});

// Sincronizar julioFcf con formData
watch(julioFcf, (newValue) => {
  if (newValue && newValue !== '') {
    formDataControlPrenatal.julioFcf = parseInt(newValue);
  } else {
    formDataControlPrenatal.julioFcf = undefined;
  }
});

// Computed para la categoría de FCF
const categoriaFcfComputed = computed(() => {
  return determinarCategoriaFcf(julioFcf.value);
});

// Validaciones reactivas
const mensajeErrorFcf = computed(() => {
  if (!julioFcf.value || julioFcf.value === '') return '';
  
  const fcf = parseInt(julioFcf.value);
...
<template>
  <div>
      ...
      <div class="mb-4">
        ...
        <div class="mb-4 mt-4">
          <div class="grid grid-cols-5 gap-3">
            <button
              ...
              :class="julioFcf === preset.valor.toString() 
        ...
        <div class="mb-4">
          <div class="flex items-center justify-center gap-3">
            <label for="julioFcf" class="font-medium text-gray-700">Personalizar:</label>
            <input
              id="julioFcf"
              v-model="julioFcf"
              type="number"
    ...
    <div class="mb-6">
      ...
      <div v-else-if="julioFcf && mensajeErrorFcf" class="p-3 bg-red-50 border border-red-200 rounded-lg">
