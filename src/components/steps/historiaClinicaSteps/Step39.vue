...
onMounted(() => {
    // Verificar si formDataHistoriaClinica.planificacionFamiliar tiene un valor y establecerlo en planificacionFamiliar
    if (formDataHistoriaClinica.planificacionFamiliar) {
        planificacionFamiliar.value = formDataHistoriaClinica.planificacionFamiliar;
        planificacionFamiliarPregunta.value = 'Si';
    }
});
...
// Watch para establecer 'Ninguno' cuando planificacionFamiliarPregunta sea 'No' y enfocar input cuando sea 'Si'
watch(planificacionFamiliarPregunta, async (newValue) => {
    if (newValue === 'No') {
        formDataHistoriaClinica.planificacionFamiliar = 'Ninguno';
    }
    if (newValue === 'Si') {
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <label 
                    :class="[
                        ...
                        planificacionFamiliarPregunta === 'No' 
                ...
                >
                    ...
                    <div 
                        :class="[
                            ...
                            planificacionFamiliarPregunta === 'No' ? 'bg-emerald-600 text-white' : 'bg-gray-200 text-gray-600'
                        ]"
                    >
                    ...
                    <span 
                        :class="[
                            ...
                            planificacionFamiliarPregunta === 'No' ? 'text-emerald-700' : 'text-gray-700'
                        ]"
                    >
                        No
                    </span>
                    ...
                    <div 
                        v-if="planificacionFamiliarPregunta === 'No'"
                ...
                <label 
                    :class="[
                        'relative flex flex-col items-center justify-center py-3 px-4 rounded-lg border-2 cursor-pointer transition-all duration-200 ease-in-out',
                        planificacionFamiliarPregunta === 'Si' 
                            ? 'border-emerald-600 bg-emerald-50 shadow-md' 
                            : 'border-gray-300 bg-white hover:border-emerald-400 hover:bg-emerald-50/50 hover:shadow-sm'
                    ]"
                >
                    ...
                    <div 
                        :class="[
                            'w-8 h-8 rounded-full flex items-center justify-center mb-1.5 transition-colors duration-200',
                            planificacionFamiliarPregunta === 'Si' ? 'bg-emerald-600 text-white' : 'bg-gray-200 text-gray-600'
                        ]"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <span 
                        :class="[
                            'text-base font-semibold transition-colors duration-200',
                            planificacionFamiliarPregunta === 'Si' ? 'text-emerald-700' : 'text-gray-700'
                        ]"
                    >
                        Sí
                    </span>
                    <!-- Indicador de selección -->
                    <div 
                        v-if="planificacionFamiliarPregunta === 'Si'"
                        class="absolute top-2 right-2 w-4 h-4 bg-emerald-600 rounded-full flex items-center justify-center"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-2.5 w-2.5 text-white" viewBox="0 0 20 20" fill="currentColor">
                            <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
