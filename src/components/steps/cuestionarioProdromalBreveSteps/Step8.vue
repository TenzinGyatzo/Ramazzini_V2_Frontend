...
function esP7Valido(v) {
  return v === SI || v === NO;
}

const p7 = computed({
  get() {
    const v = formDataCuestionarioProdromalBreve.p7;
    return esP7Valido(v) ? v : NO;
  },
  set(nuevo) {
    formDataCuestionarioProdromalBreve.p7 = nuevo;
    if (nuevo !== SI) {
      formDataCuestionarioProdromalBreve.p7GradoAcuerdoStatement = undefined;
    }
  },
});

const p7GradoAcuerdoStatement = computed({
  get() {
    const v = formDataCuestionarioProdromalBreve.p7GradoAcuerdoStatement;
    return GRADO_ACUERDO_OPCIONES.includes(v) ? v : '';
  },
  set(nuevo) {
    if (formDataCuestionarioProdromalBreve.p7 !== SI) {
      formDataCuestionarioProdromalBreve.p7GradoAcuerdoStatement = undefined;
      return;
    }
    if (GRADO_ACUERDO_OPCIONES.includes(nuevo)) {
      formDataCuestionarioProdromalBreve.p7GradoAcuerdoStatement = nuevo;
    } else {
      formDataCuestionarioProdromalBreve.p7GradoAcuerdoStatement = undefined;
    }
  },
...
const mostrarGrado = computed(() => p7.value === SI);

function asegurarP7DefaultEnStore() {
  if (!esP7Valido(formDataCuestionarioProdromalBreve.p7)) {
    formDataCuestionarioProdromalBreve.p7 = NO;
  }
}

onMounted(asegurarP7DefaultEnStore);
asegurarP7DefaultEnStore();
</script>

<template>
  <div>
    <h1 class="text-2xl font-bold mb-2 text-gray-900">Cuestionario Prodromal Breve</h1>
    <p class="text-sm text-gray-600 mb-6">Pregunta 7 de 21</p>
    ...
    <div class="mb-6">
      ...
      <div class="grid grid-cols-2 gap-3 max-w-md">
        ...
        >
          <input type="radio" :value="NO" v-model="p7" class="sr-only" />
          <div
            :class="[
              'w-8 h-8 rounded-full flex items-center justify-center mb-1.5 transition-colors duration-200',
              p7 === NO ? 'bg-emerald-600 text-white' : 'bg-gray-200 text-gray-600',
            ]"
          >
          ...
          <span
            :class="[
              ...
              p7 === NO ? 'text-emerald-700' : 'text-gray-700',
            ]"
          >
            No
          </span>
        ...
        >
          <input type="radio" :value="SI" v-model="p7" class="sr-only" />
          <div
            :class="[
              'w-8 h-8 rounded-full flex items-center justify-center mb-1.5 transition-colors duration-200',
              p7 === SI ? 'bg-emerald-600 text-white' : 'bg-gray-200 text-gray-600',
            ]"
          >
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <span
            :class="[
              'text-base font-semibold transition-colors duration-200',
              p7 === SI ? 'text-emerald-700' : 'text-gray-700',
            ]"
          >
            Sí
          </span>
        ...
        <div class="flex flex-col gap-2">
          ...
          >
            <input
              type="radio"
              :value="opt"
              v-model="p7GradoAcuerdoStatement"
              class="h-4 w-4 text-emerald-600 border-gray-300 focus:ring-emerald-500"
            />
            <span
              :class="[
                'text-sm leading-snug',
                p7GradoAcuerdoStatement === opt ? 'font-semibold text-emerald-800' : 'text-gray-700',
              ]"
            >
              {{ opt }}
            </span>
