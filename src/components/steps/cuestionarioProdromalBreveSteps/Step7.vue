...
function esP6Valido(v) {
  return v === SI || v === NO;
}

const p6 = computed({
  get() {
    const v = formDataCuestionarioProdromalBreve.p6;
    return esP6Valido(v) ? v : NO;
  },
  set(nuevo) {
    formDataCuestionarioProdromalBreve.p6 = nuevo;
    if (nuevo !== SI) {
      formDataCuestionarioProdromalBreve.p6GradoAcuerdoStatement = undefined;
    }
  },
});

const p6GradoAcuerdoStatement = computed({
  get() {
    const v = formDataCuestionarioProdromalBreve.p6GradoAcuerdoStatement;
    return GRADO_ACUERDO_OPCIONES.includes(v) ? v : '';
  },
  set(nuevo) {
    if (formDataCuestionarioProdromalBreve.p6 !== SI) {
      formDataCuestionarioProdromalBreve.p6GradoAcuerdoStatement = undefined;
      return;
    }
    if (GRADO_ACUERDO_OPCIONES.includes(nuevo)) {
      formDataCuestionarioProdromalBreve.p6GradoAcuerdoStatement = nuevo;
    } else {
      formDataCuestionarioProdromalBreve.p6GradoAcuerdoStatement = undefined;
    }
  },
...
const mostrarGrado = computed(() => p6.value === SI);

function asegurarP6DefaultEnStore() {
  if (!esP6Valido(formDataCuestionarioProdromalBreve.p6)) {
    formDataCuestionarioProdromalBreve.p6 = NO;
  }
}

onMounted(asegurarP6DefaultEnStore);
asegurarP6DefaultEnStore();
</script>

<template>
  <div>
    <h1 class="text-2xl font-bold mb-2 text-gray-900">Cuestionario Prodromal Breve</h1>
    <p class="text-sm text-gray-600 mb-6">Pregunta 6 de 21</p>
    ...
    <div class="mb-6">
      ...
      <div class="grid grid-cols-2 gap-3 max-w-md">
        ...
        >
          <input type="radio" :value="NO" v-model="p6" class="sr-only" />
          <div
            :class="[
              'w-8 h-8 rounded-full flex items-center justify-center mb-1.5 transition-colors duration-200',
              p6 === NO ? 'bg-emerald-600 text-white' : 'bg-gray-200 text-gray-600',
            ]"
          >
          ...
          <span
            :class="[
              ...
              p6 === NO ? 'text-emerald-700' : 'text-gray-700',
            ]"
          >
            No
          </span>
        ...
        >
          <input type="radio" :value="SI" v-model="p6" class="sr-only" />
          <div
            :class="[
              'w-8 h-8 rounded-full flex items-center justify-center mb-1.5 transition-colors duration-200',
              p6 === SI ? 'bg-emerald-600 text-white' : 'bg-gray-200 text-gray-600',
            ]"
          >
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <span
            :class="[
              'text-base font-semibold transition-colors duration-200',
              p6 === SI ? 'text-emerald-700' : 'text-gray-700',
            ]"
          >
            Sí
          </span>
        ...
        <div class="flex flex-col gap-2">
          ...
          >
            <input
              type="radio"
              :value="opt"
              v-model="p6GradoAcuerdoStatement"
              class="h-4 w-4 text-emerald-600 border-gray-300 focus:ring-emerald-500"
            />
            <span
              :class="[
                'text-sm leading-snug',
                p6GradoAcuerdoStatement === opt ? 'font-semibold text-emerald-800' : 'text-gray-700',
              ]"
            >
              {{ opt }}
            </span>
