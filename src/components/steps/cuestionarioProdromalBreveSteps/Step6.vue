...
function esP5Valido(v) {
  return v === SI || v === NO;
}

const p5 = computed({
  get() {
    const v = formDataCuestionarioProdromalBreve.p5;
    return esP5Valido(v) ? v : NO;
  },
  set(nuevo) {
    formDataCuestionarioProdromalBreve.p5 = nuevo;
    if (nuevo !== SI) {
      formDataCuestionarioProdromalBreve.p5GradoAcuerdoStatement = undefined;
    }
  },
});

const p5GradoAcuerdoStatement = computed({
  get() {
    const v = formDataCuestionarioProdromalBreve.p5GradoAcuerdoStatement;
    return GRADO_ACUERDO_OPCIONES.includes(v) ? v : '';
  },
  set(nuevo) {
    if (formDataCuestionarioProdromalBreve.p5 !== SI) {
      formDataCuestionarioProdromalBreve.p5GradoAcuerdoStatement = undefined;
      return;
    }
    if (GRADO_ACUERDO_OPCIONES.includes(nuevo)) {
      formDataCuestionarioProdromalBreve.p5GradoAcuerdoStatement = nuevo;
    } else {
      formDataCuestionarioProdromalBreve.p5GradoAcuerdoStatement = undefined;
    }
  },
...
const mostrarGrado = computed(() => p5.value === SI);

function asegurarP5DefaultEnStore() {
  if (!esP5Valido(formDataCuestionarioProdromalBreve.p5)) {
    formDataCuestionarioProdromalBreve.p5 = NO;
  }
}

onMounted(asegurarP5DefaultEnStore);
asegurarP5DefaultEnStore();
</script>

<template>
  <div>
    <h1 class="text-2xl font-bold mb-2 text-gray-900">Cuestionario Prodromal Breve</h1>
    <p class="text-sm text-gray-600 mb-6">Pregunta 5 de 21</p>
    ...
    <div class="mb-6">
      ...
      <div class="grid grid-cols-2 gap-3 max-w-md">
        ...
        >
          <input type="radio" :value="NO" v-model="p5" class="sr-only" />
          <div
            :class="[
              'w-8 h-8 rounded-full flex items-center justify-center mb-1.5 transition-colors duration-200',
              p5 === NO ? 'bg-emerald-600 text-white' : 'bg-gray-200 text-gray-600',
            ]"
          >
          ...
          <span
            :class="[
              ...
              p5 === NO ? 'text-emerald-700' : 'text-gray-700',
            ]"
          >
            No
          </span>
        ...
        >
          <input type="radio" :value="SI" v-model="p5" class="sr-only" />
          <div
            :class="[
              'w-8 h-8 rounded-full flex items-center justify-center mb-1.5 transition-colors duration-200',
              p5 === SI ? 'bg-emerald-600 text-white' : 'bg-gray-200 text-gray-600',
            ]"
          >
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <span
            :class="[
              'text-base font-semibold transition-colors duration-200',
              p5 === SI ? 'text-emerald-700' : 'text-gray-700',
            ]"
          >
            Sí
          </span>
        ...
        <div class="flex flex-col gap-2">
          ...
          >
            <input
              type="radio"
              :value="opt"
              v-model="p5GradoAcuerdoStatement"
              class="h-4 w-4 text-emerald-600 border-gray-300 focus:ring-emerald-500"
            />
            <span
              :class="[
                'text-sm leading-snug',
                p5GradoAcuerdoStatement === opt ? 'font-semibold text-emerald-800' : 'text-gray-700',
              ]"
            >
              {{ opt }}
            </span>
