...
function obtenerInicial() {
  const desdeForm = formDataTrastornosEstadoAnimo.p1MasSocialExtrovertido;
  if (desdeForm === SI || desdeForm === NO) {
    return desdeForm;
  }
  const desdeDoc = documentos.currentDocument?.p1MasSocialExtrovertido;
...
const p1MasSocialExtrovertido = ref(obtenerInicial());

watch(
  p1MasSocialExtrovertido,
  (nuevo) => {
    formDataTrastornosEstadoAnimo.p1MasSocialExtrovertido = nuevo;
  },
  { immediate: true }
);
</script>
      ...
      <div class="grid grid-cols-2 gap-3">
        ...
        >
          <input
            type="radio"
            :value="NO"
            v-model="p1MasSocialExtrovertido"
            class="sr-only"
          />
          <div
            :class="[
              'w-8 h-8 rounded-full flex items-center justify-center mb-1.5 transition-colors duration-200',
              p1MasSocialExtrovertido === NO ? 'bg-emerald-600 text-white' : 'bg-gray-200 text-gray-600',
            ]"
          >
          ...
          <span
            :class="[
              ...
              p1MasSocialExtrovertido === NO ? 'text-emerald-700' : 'text-gray-700',
            ]"
          >
            No
          </span>
          <div
            v-if="p1MasSocialExtrovertido === NO"
        ...
        >
          ...
          <div
            :class="[
              'w-8 h-8 rounded-full flex items-center justify-center mb-1.5 transition-colors duration-200',
              p1MasSocialExtrovertido === SI ? 'bg-emerald-600 text-white' : 'bg-gray-200 text-gray-600',
            ]"
          >
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <span
            :class="[
              'text-base font-semibold transition-colors duration-200',
              p1MasSocialExtrovertido === SI ? 'text-emerald-700' : 'text-gray-700',
            ]"
          >
            Sí
          </span>
          <div
            v-if="p1MasSocialExtrovertido === SI"
            class="absolute top-2 right-2 w-4 h-4 bg-emerald-600 rounded-full flex items-center justify-center"
          >
            <svg xmlns="http://www.w3.org/2000/svg" class="h-2.5 w-2.5 text-white" viewBox="0 0 20 20" fill="currentColor">
              <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
